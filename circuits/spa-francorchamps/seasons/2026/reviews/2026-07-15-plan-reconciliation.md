...
|---|---|---|---|---|---|---|---|---|---|
| POSTFINAL-01 | `PRRT_kwDOTZdGvM6RhvFf / PRRC_kwDOTZdGvM7WbI8S` | accepted | nullable braking targets are filtered by observed mask and finite target before encoding or statistics; insufficient support fails closed | paired feature/mask → outcome frame → encoder/PyMC | yes | pending | pending | pending | pending |
| POSTFINAL-02 | `PRRT_kwDOTZdGvM6RhvFg / PRRC_kwDOTZdGvM7WbI8T` | accepted with modification | field-best serialization uses exact profile/draw/attempt `AttemptEvidence` for lap, sectors, trace, and timing | field sample identity → evidence store → artifact | yes | pending | pending | pending | pending |
| POSTFINAL-03 | `PRRT_kwDOTZdGvM6RhvFk / PRRC_kwDOTZdGvM7WbI8a` | accepted | positive release fixture contains the exact report set, passing nested checks, full intervals/evidence/scenario/provenance/checksums | fixture → builder → semantic validator | yes | pending | pending | pending | pending |
| POSTFINAL-04 | `PRRT_kwDOTZdGvM6RhvFm / PRRC_kwDOTZdGvM7WbI8d` | accepted | artifact module imports `UTC` with `datetime` so default validation and CLI calls execute | direct/CLI call → validator default clock | yes | pending | pending | pending | pending |
| POSTFINAL-05 | `PRRT_kwDOTZdGvM6RhvFo / PRRC_kwDOTZdGvM7WbI8f` | accepted | human geometry-source config is frozen deterministically into consumed manifest rows with path, format, and measured digest | source config → freeze tool → circuits manifest → loader | yes | pending | pending | pending | pending |
| POSTFINAL-06 | `PRRT_kwDOTZdGvM6RhvFs / PRRC_kwDOTZdGvM7WbI8m` | accepted | candidate lap limits and Q3/SQ3 fallback are applied per stable car-driver profile | lap source → profile selection → corpus | yes | pending | pending | pending | pending |
| POSTFINAL-07 | `PRRT_kwDOTZdGvM6RhvFx / PRRC_kwDOTZdGvM7WbI8s` | accepted | duplicate anchors are normalized; unsupported zero-length phases are omitted; windows remain ordered and non-overlapping | anchors → phase windows → features | yes | pending | pending | pending | pending |
| POSTFINAL-08 | `PRRT_kwDOTZdGvM6RhvF0 / PRRC_kwDOTZdGvM7WbI8x` | accepted with modification | raw analogue consumer uses `analogue_phase_id`; paired-delta frames remain invalid analogue evidence | raw features → analogue adapter → consumer | yes | pending | pending | pending | pending |
| POSTFINAL-09 | `PRRT_kwDOTZdGvM6RhvF4 / PRRC_kwDOTZdGvM7WbI81` | accepted | release reports require exact canonical keys, top-level pass, and all nested checks true | validation reports → release predicate | yes | pending | pending | pending | pending |
| POSTFINAL-10 | `PRRT_kwDOTZdGvM6RhvF7 / PRRC_kwDOTZdGvM7WbI85` | accepted | browser chronology enforces sourceCutoff <= generatedAt <= validationNow plus maximum age | artifact timestamps → browser validator | yes | pending | pending | pending | pending |
| POSTFINAL-11 | `PRRT_kwDOTZdGvM6RhvF_ / PRRC_kwDOTZdGvM7WbI88` | accepted | durable status records evidence-sync head/CI and 13-finding merge block | GitHub/CI/review evidence → status/next action | yes | pending | pending | pending | pending |
| POSTFINAL-12 | `PRRT_kwDOTZdGvM6RhvGD / PRRC_kwDOTZdGvM7WbI9F` | accepted | end-to-end fixture passes source, circuit-year, and lap-selection manifests matching the builder signature | fixture paths → corpus builder → provenance | yes | pending | pending | pending | pending |
| POSTFINAL-13 | `PRRT_kwDOTZdGvM6RhvGG / PRRC_kwDOTZdGvM7WbI9I` | accepted | required checksum fields and exact report-checksum coverage use lowercase 64-hex validation before release/checksum finalization | provenance → builder/validator → release | yes | pending | pending | pending | pending |
