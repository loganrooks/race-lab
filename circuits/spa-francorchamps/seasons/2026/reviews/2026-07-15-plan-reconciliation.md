...
|---|---|---|---|---|---|---|---|---|---|
| FINAL-01 | `PRRT_kwDOTZdGvM6RRUfK` / `3591396962` | accepted | `alignment.py` annotated `RawCoverage` without importing it. Plan A imports the type and uses it at the alignment boundary. | quality → alignment → pipeline | yes | pending CI | pending CI | pending CI | local targeted review complete; final review not requested |
| FINAL-02 | `PRRT_kwDOTZdGvM6RRUfL` / `3591396965` | accepted | Human geometry-source rows and the generated circuit manifest were conflated. Plan A now declares source path/format in `geometry-sources.yaml` and deterministically freezes exact-byte SHA-256 values into `circuits.yaml`; no invented digest is accepted. | source config → manifest generator → loader → corpus provenance | yes | pending CI | pending CI | pending CI | local targeted review complete; final review not requested |
| FINAL-03 | `PRRT_kwDOTZdGvM6RRUfN` / `3591396968` | accepted with modification | Report nested checks were conjoined while report-level `passed` was ignored. Plan B defines one required-report set, rejects duplicates/missing/unexpected names, and conjoins report flags plus all nested checks; Plan C mirrors the predicate. | validation reports → artifact builder/validator → generator/browser | yes | pending CI | pending CI | pending CI | Python/browser report-name parity reviewed; final review not requested |
| FINAL-04 | `PRRT_kwDOTZdGvM6RRUfP` / `3591396971` | accepted with modification | Team-only identity could collide across drivers and select unrelated evidence. Plan B preserves `profile_id`, team, driver, draw, attempt, lap time, and sectors through `AttemptKey` and verifies the representative evidence before serialization. | profile sampling → field minimum → trace summary → artifact | yes | pending CI | pending CI | pending CI | identity flow reviewed end to end; final review not requested |
| FINAL-05 | `PRRT_kwDOTZdGvM6RRUfU` / `3591396977` | accepted | Browser validation checked only the 80% triple. Plan C requires finite ordered `lower95`, `lower80`, `median`, `upper80`, `upper95` for field best while intentionally retaining the ordered three-point team contract. | Python interval producer → Python validator → browser validator/UI | yes | pending CI | pending CI | pending CI | interval shapes compared across plans; final review not requested |
| FINAL-06 | `PRRT_kwDOTZdGvM6RRUfY` / `3591396981` | accepted with modification | Attempt variation changed summary values but retained the unvaried base trace. Plan B stores trace and timing from each varied attempt under the exact profile/draw/attempt key and checks trace endpoint against the sampled lap. | attempt variation → evidence store → team/field representative → artifact | yes | pending CI | pending CI | pending CI | exact-attempt evidence path reviewed; final review not requested |
| FINAL-07 | `PRRT_kwDOTZdGvM6RRUfZ` / `3591396984` | accepted | A released field summary could omit arrays consumed by source construction. Plans B and C require at least two valid trace/timing rows, matching row counts/progress, and timing fractions derived from the same trace; invalid artifacts fail closed. | Python serializer/validator → generator → source selector/playback | yes | pending CI | pending CI | pending CI | shared trace/timing contract reviewed; final review not requested |
| FINAL-08 | `PRRT_kwDOTZdGvM6RRUfb` / `3591396987` | accepted | Status body/front matter described incompatible publication states. `STATUS.md` is synchronized to the directly observed `76b8226e...` head, 22 unresolved current threads, and a local-unpublished first publication unit. | GitHub observation → status → next action | yes | pending CI | pending CI | pending CI | direct remote evidence recorded; final review not requested |
| FINAL-09 | `PRRT_kwDOTZdGvM6RRUfd` / `3591396989` | accepted | Geometry checksum provenance was recorded without verifying file bytes. Plan A hashes exact bytes and rejects mismatch before CSV/JSON/adapter parsing. | manifest → byte reader → canonical geometry → corpus provenance | yes | pending CI | pending CI | pending CI | pre-parse digest order reviewed; final review not requested |
| FINAL-10 | `PRRT_kwDOTZdGvM6RRUfe` / `3591396990` | accepted with modification | Raw features contained observed year but paired deltas contained `pair_id`; neither was a valid implicit analogue schema. Plan B adds an explicit raw-feature adapter with stable observed phase identity/year and rejects paired-delta frames. | Plan A raw features → analogue evidence → nearest analogues → browser provenance | yes | pending CI | pending CI | pending CI | raw/paired interfaces kept distinct; final review not requested |
| FINAL-11 | `PRRT_kwDOTZdGvM6RRUfh` / `3591396994` | accepted | The control validator searched all historical activity and could reuse an old lesson sentinel. A regression was added first and observed RED; the validator now checks only the latest activity entry and the focused/full tests are GREEN. | changed-file discovery → activity parsing → lesson-update enforcement | yes | pending CI | pending CI | pending CI | TDD RED/GREEN evidence retained in activity; final review not requested |
| FINAL-12 | `PRRT_kwDOTZdGvM6RRg3e` / `3591467142` | accepted | A `CanonicalTrack` fixture omitted required source provenance. Plan A supplies a complete `GeometrySource` with path, format, checksum, source name, and license. | geometry fixture → loader/resampling test | yes | pending CI | pending CI | pending CI | complete fixture construction reviewed; final review not requested |
| FINAL-13 | `PRRT_kwDOTZdGvM6RRg3h` / `3591467149` | accepted with modification | Public calls omitted keyword-only `now`. Plan B defaults direct validation to current UTC while builders/tests/CLI inject explicit UTC; Plan C likewise accepts an injected valid `Date` with a safe default. | builder/test/CLI/direct callers → validator clock → freshness | yes | pending CI | pending CI | pending CI | clock call sites searched; final review not requested |
| FINAL-14 | `PRRT_kwDOTZdGvM6RRg3k` / `3591467151` | accepted with modification | Closeout completion was hard-coded to stale review counts. The closeout plan and this ledger use stable `FINAL-01` through `FINAL-22` rows; publication/action/final-review gates derive from row state rather than prose totals. | review inventory → plan tasks → status/action ledger | yes | pending CI | pending CI | pending CI | task IDs and status references checked; final review not requested |
| FINAL-15 | `PRRT_kwDOTZdGvM6RRg3n` / `3591467157` | accepted | Quality fixtures constructed `LapContext` without pre-normalization coverage. Plan A adds a reusable `raw_coverage()` fixture and passes it at every construction. | source trace → fixture coverage → classifier | yes | pending CI | pending CI | pending CI | every shown constructor searched; final review not requested |
| FINAL-16 | `PRRT_kwDOTZdGvM6RRg3p` / `3591467159` | accepted | Seeding maximum gap with full lap duration rejected normal laps. Plan A computes only consecutive observed timestamp gaps and uses a fail-closed fallback when no interval exists. | raw timestamps → `RawCoverage.maximum_gap_s` → quality gate | yes | pending CI | pending CI | pending CI | observed-interval calculation reviewed; final review not requested |
| FINAL-17 | `PRRT_kwDOTZdGvM6RRg3u` / `3591467165` | accepted with modification | Nullable braking onsets entered an ordinary weighted mean. Plan A aggregates only paired observed values and emits `braking_onset_pair_observed`; Plan B filters that outcome by the mask and never zero-fills missing residuals. | phase extraction → paired feature → design matrix → validation metrics | yes | pending CI | pending CI | pending CI | nullable producer/consumer chain reviewed; final review not requested |
| FINAL-18 | `PRRT_kwDOTZdGvM6RRg3z` / `3591467170` | accepted | The claimed released fixture omitted required release evidence. Plans B and C now use complete fixtures with canonical reports, actual hexadecimal SHA-256 values, provenance, scenario, trace/timing, intervals, and representative identity; negatives mutate one property. | test fixture → builder/validator → browser fixture module | yes | pending CI | pending CI | pending CI | fixture corpus parity reviewed; final review not requested |
| FINAL-19 | `PRRT_kwDOTZdGvM6RRg32` / `3591467173` | accepted | A bare Typer parameter was positional while documentation used `--artifact`. Plan B declares a required readable file option and keeps the documented invocation. | CLI signature → command documentation → validator | yes | pending CI | pending CI | pending CI | CLI call contract reviewed; final review not requested |
| FINAL-20 | `PRRT_kwDOTZdGvM6RRg34` / `3591467175` | accepted with modification | Corpus verification trusted whatever filenames the manifest declared. Plan A requires exact equality among the canonical four-file set, manifest keys, directory parquet files, regular-file checks, and per-file digests. | writer → manifest → filesystem → verifier | yes | pending CI | pending CI | pending CI | set/digest ordering reviewed; final review not requested |
| FINAL-21 | `PRRT_kwDOTZdGvM6RRg37` / `3591467178` | accepted | Lap acceptance read reversal after monotonic repair. Plan A uses `context.raw_coverage.maximum_progress_reversal`, so normalization cannot erase source defects. | raw projection → coverage → repair/resampling → acceptance | yes | pending CI | pending CI | pending CI | raw-versus-repaired search complete; final review not requested |
| FINAL-22 | `PRRT_kwDOTZdGvM6RRg3-` / `3591467181` | accepted | PyYAML may return ISO strings, `date`, or `datetime`. Plan A normalizes all configuration/weather/control/lap/session timestamp forms to aware UTC at the boundary. | YAML/API values → UTC normalization → event/weather/control/cutoff consumers | yes | pending CI | pending CI | pending CI | timestamp boundary call sites searched; final review not requested |
